...
try {
  ...
  const clearEvents = () => evaluate(client, `window.__joyrideSpike.clearEvents()`);
  ...
  check("overlay: portal + overlay + spotlight svg mounted, floater visible", ov.portalPresent && ov.overlayPresent && ov.pathCount >= 2 && ov.floaterPresent && ov.floaterOpacity === "1", {
    pathCount: ov.pathCount, svgPosition: ov.svgPosition, overlayPosition: ov.overlayPosition,
  });
  check("overlay: options.zIndex 45 -> overlay z-index 45, floater 46", ov.overlayZIndex === "45" && ov.floaterZIndex === "46", {
    overlayZIndex: ov.overlayZIndex, floaterZIndex: ov.floaterZIndex,
  });
  check("overlay: wrapper pointer-events none, cover path auto, cutout path none (blockTargetInteraction:false)", ov.overlayPointerEvents === "none" && ov.coverPath.pointerEvents === "auto" && ov.cutoutPath.pointerEvents === "none", {
    overlay: ov.overlayPointerEvents, cover: ov.coverPath.pointerEvents, cutout: ov.cutoutPath.pointerEvents,
  });
  check("overlay: hole = composer rect + spotlightPadding 8, radius 8", rectsMatch(ov.cutout, composer, 8) && ov.cutout.radius === 8, {
    cutout: ov.cutout, composer: { left: composer.left, top: composer.top, width: composer.width, height: composer.height },
  });
  ...
  check("focus: presenting with disableFocusTrap does not steal focus (activeElement is body)", ov.activeElement === "body" && !ov.activeInsideFloater, { activeElement: ov.activeElement });
  const startEvents = await events();
  check("events: tour:start, step:before, tooltip fired for index 0 with controlled:true", startEvents.some((e) => e.type === "tour:start") && startEvents.some((e) => e.type === "step:before" && e.index === 0) && startEvents.some((e) => e.type === "tooltip" && e.index === 0) && startEvents.every((e) => e.controlled), { events: startEvents.map((e) => `${e.type}/${e.action}/${e.index}`) });
  ...
  check("hit-test: exposed target (Send) is under the pointer through the hole and a real click reaches it", sendHit === "spike:send" && p.sendClicks === "1", { sendHit, sendClicks: p.sendClicks });
  check("hit-test: control outside the hole is covered by the overlay path; click swallowed, tour untouched (overlayClickAction:false)", coveredHit === "joyride-overlay-path" && p.coveredClicks === "0" && p.stepIndex === "0" && p.run === "true" && !afterCoveredClick.some((e) => e.action === "close" || e.type === "step:after"), { coveredHit, coveredRect: covered, coveredClicks: p.coveredClicks, stepIndex: p.stepIndex });
  ...
  await delay(200);
  p = await probe();
  check("sonner: toast action is under the pointer above the tour overlay and a real click registers", toastHit === "spike:toast-action" && p.toastClicks === "1", { toastHit, toastClicks: p.toastClicks });
  ...
  p = await probe();
  check("keyboard: click in the hole focuses the editor, typing lands there, Tab reaches Send", p.editorValue === "hello" && ov.activeElement === "send", { editorValue: p.editorValue, activeAfterTab: ov.activeElement });
  await evaluate(client, `document.querySelector('[data-action="primary"]').focus()`);
  await pressTab();
  await delay(100);
  ov = await readOverlay();
  check("keyboard: disableFocusTrap:true - Tab from the card's last button LEAVES the card (no trap)", !ov.activeInsideFloater, { activeAfterTab: ov.activeElement, insideFloater: ov.activeInsideFloater });
  // Control arm: the trap ON must behave differently, or the check above is vacuous.
  await control("setDisableFocusTrap(false)");
  await delay(300);
  ov = await readOverlay();
  const trapAutofocused = ov.activeElement === "primary" && ov.activeInsideFloater;
  await evaluate(client, `document.querySelector('[data-action="primary"]').focus()`);
  await pressTab();
  await delay(100);
  ov = await readOverlay();
  check("keyboard (control arm): disableFocusTrap:false autofocuses Next and Tab wraps INSIDE the card", trapAutofocused && ov.activeInsideFloater, { autofocusedPrimary: trapAutofocused, activeAfterTab: ov.activeElement, insideFloater: ov.activeInsideFloater });
  ...
  check("controlled: Next -> step:after {action:next,index:0,origin:button_primary} -> app sets stepIndex 1 -> step 1 presents", stepAfter !== undefined && stepAfter.action === "next" && stepAfter.index === 0 && stepAfter.origin === "button_primary" && p.stepIndex === "1" && nextEvents.some((e) => e.type === "tooltip" && e.index === 1), { stepAfter, stepIndex: p.stepIndex, events: nextEvents.map((e) => `${e.type}/${e.action}/${e.index}`) });
  ov = await readOverlay();
  let column = await rectOf('[data-visible="true"] [data-spike="column"]');
  check("function target: step 1 resolved the VISIBLE surface's column (hidden duplicate ignored); hole matches", rectsMatch(ov.cutout, column, 8), { cutout: ov.cutout, column });
  ...
  check("dialog: run=false while a modal is presented -> overlay and card unmounted", !ov.overlayPresent && !ov.floaterPresent && p.run === "false" && p.stepIndex === "1", { overlayPresent: ov.overlayPresent, floaterPresent: ov.floaterPresent, run: p.run });
  observe("dialog: events emitted by the suspension itself (run true->false) - note any step:after and its action", { events: suspendEvents.map((e) => `${e.type}/${e.action}/${e.index}/${e.status}`) });
  ...
  await delay(100);
  const dialogInputValue = await evaluate(client, `document.querySelector('[data-spike="dialog-input"]').value`);
  check("dialog: z-50 Dialog is usable while the tour is suspended (click + type)", dialogHit === "spike:dialog-input" && dialogInputValue === "/tmp", { dialogHit, dialogInputValue });
  ...
  ov = await readOverlay();
  p = await probe();
  check("dialog: closing the NESTED dialog does not resume the tour while the outer one is presented", !ov.overlayPresent && p.run === "false" && p.modalCount === "1", { overlayPresent: ov.overlayPresent, modalCount: p.modalCount });
  ...
  const dialogGoneSync = await evaluate(client, `document.querySelector('[data-spike="dialog"]') === null`);
  observe("dialog (sync-resume arm): Esc closes the Dialog - does the SAME keystroke also reach the just-resumed tour as a CLOSE?", { dialogGone: dialogGoneSync, tourStatus: p.tourStatus, run: p.run, stepIndex: p.stepIndex, events: escSyncEvents.map((e) => `${e.type}/${e.action}/${e.index}/${e.status}`) });
  ...
  const dialogGone = await evaluate(client, `document.querySelector('[data-spike="dialog"]') === null`);
  check("dialog (deferred-resume arm): Esc closes the Dialog only; tour resumes the SAME step, no CLOSE from the tour", dialogGone && p.dialogOpen === "false" && p.run === "true" && p.stepIndex === "1" && p.tourStatus === "active" && !escEvents.some((e) => e.action === "close"), { dialogGone, stepIndex: p.stepIndex, run: p.run, tourStatus: p.tourStatus, events: escEvents.map((e) => `${e.type}/${e.action}/${e.index}`) });
  await waitForTooltip("2 of 4");
  await delay(300);
  ov = await readOverlay();
  column = await rectOf('[data-visible="true"] [data-spike="column"]');
  check("dialog: after resume the hole is back on the step-1 target", rectsMatch(ov.cutout, column, 8), { cutout: ov.cutout });
  observe("dialog: events emitted by resume (run false->true at the same stepIndex)", { events: escEvents.map((e) => `${e.type}/${e.action}/${e.index}/${e.status}`) });
  ...
  check("esc: dismissKeyAction close -> step:after {action:close, origin:keyboard} -> app pauses, stepIndex unchanged, overlay gone", escStepAfter !== undefined && escStepAfter.action === "close" && escStepAfter.origin === "keyboard" && p.tourStatus === "paused" && p.stepIndex === "1" && !ov.overlayPresent, { escStepAfter, tourStatus: p.tourStatus, stepIndex: p.stepIndex, overlayPresent: ov.overlayPresent });
  await control("setRun(true)");
  await waitForTooltip("2 of 4");
  ...
  await delay(200);
  let scrollEvents = (await events()).filter((e) => e.type.startsWith("scroll:"));
  const normalScroll = scrollEvents.length >= 2 ? { duration: scrollEvents[0].scrollDuration, elapsedMs: scrollEvents[scrollEvents.length - 1].at - scrollEvents[0].at, scrollY: (await readOverlay()).scrollY } : null;
  check("scroll: step 2 (below the fold) scrolled with scrollDuration 300 under normal motion", normalScroll !== null && normalScroll.duration === 300 && normalScroll.scrollY > 0, normalScroll);
  ...
  await delay(400);
  ov = await readOverlay();
  check("reduced motion: harness reads prefers-reduced-motion via motion/react useReducedMotion", ov.tooltip !== null, { reducedMotion: (await probe()).reducedMotion });
  ...
  const reducedScroll = scrollEvents.length >= 2 ? { duration: scrollEvents[0].scrollDuration, elapsedMs: scrollEvents[scrollEvents.length - 1].at - scrollEvents[0].at } : null;
  check("reduced motion: scrollDuration 0 -> scroll:start/end report duration 0", reducedScroll !== null && reducedScroll.duration === 0, reducedScroll);
  observe("reduced motion: scroll wall time normal vs reduced (upstream floors any <100px scroll at 50ms)", { normal: normalScroll, reduced: reducedScroll });
  ...
  check("narrow viewport 480x600: card (placement top, composer target) stays fully inside the viewport", fr !== null && fr.left >= 0 && fr.top >= 0 && fr.right <= 480 && fr.bottom <= 600, { floaterRect: fr, scrollY: ov.scrollY });
  ...
  check("narrow viewport 480x600: card (placement right, column target) stays fully inside the viewport (flip/shift)", fr1 !== null && fr1.left >= 0 && fr1.top >= 0 && fr1.right <= 480 && fr1.bottom <= 600, { floaterRect: fr1, cutout: ov.cutout });
  ...
  p = await probe();
  ov = await readOverlay();
  const loaderAt600 = await evaluate(client, `Boolean(document.querySelector('[data-testid="loader"], .react-joyride__loader'))`);
  observe("missing target: state 600ms into the 8s wait (loader shown after loaderDelay 300ms?)", { stepIndex: p.stepIndex, overlayPresent: ov.overlayPresent, pathCount: ov.pathCount, floaterPresent: ov.floaterPresent, loaderPresent: loaderAt600, targetNotFound: p.targetNotFound });
  ...
  check("skip: Skip -> tour:end {action:skip, status:skipped} -> app ends, overlay gone", skipEvents.some((e) => e.type === "tour:end" && e.action === "skip" && e.status === "skipped") && p.tourStatus === "skipped" && !ov.overlayPresent, { tourStatus: p.tourStatus, events: skipEvents.map((e) => `${e.type}/${e.action}/${e.status}`) });
  observe("skip: does Skip emit a step:after at all? (plan wires ACTIONS.SKIP off step:after)", { stepAfterEmitted: skipEvents.some((e) => e.type === "step:after"), events: skipEvents.map((e) => `${e.type}/${e.action}/${e.status}`) });
  ...
  await delay(200);
  p = await probe();
  check("hit-test (control arm): with the overlay gone the covered control receives the same click", p.coveredClicks === "1", { coveredClicks: p.coveredClicks });
  ...
  if (chromeProfilePath !== undefined) {
    await rm(chromeProfilePath, { recursive: true, force: true, maxRetries: 3 });
  }
}
