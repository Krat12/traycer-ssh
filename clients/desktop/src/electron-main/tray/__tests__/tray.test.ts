...
describe("DesktopTrayController menu structure", () => {
  ...
  it("renders a disabled 'No recent epics' item when the epic list is empty", () => {
    ...
    expect(labels).toContain("Open Traycer SSH Dev");
    expect(labels).toContain("Quit Traycer SSH Dev");
  ...
  it("does not render a 'Status:' row (status drives the tooltip instead)", () => {
    ...
    expect(tray.toolTips[tray.toolTips.length - 1]).toBe(
      "Traycer SSH Dev (attention)",
    );
  });

  it("renders epic rows under separators alongside Open Traycer SSH Dev and Quit Traycer SSH Dev", () => {
    const controller = new DesktopTrayController(makeWindow(), trayImage(), {
      onEpicSelected: null,
      onCommand: null,
    });
    ...
    expect(labels[0]).toBe("Open Traycer SSH Dev");
    expect(labels).toContain("First Epic");
    expect(labels).toContain("Second Epic");
    expect(labels[labels.length - 1]).toBe("Quit Traycer SSH Dev");
  ...
  it("opens epics on the current MRU window through the managed-window seam", () => {
    ...
    const openRow = firstTemplate.find(
      (entry) => entry.label === "Open Traycer SSH Dev",
    );
    if (openRow?.click === undefined) {
      throw new Error(
        "expected Open Traycer SSH Dev row to have a click handler",
      );
    }
    ...
    // Two Open Traycer SSH Dev clicks (one per MRU window) plus the epic click landing
  ...
  // Decision 9: the "Open Traycer SSH Dev" item is display-only for the summon
  ...
  it("shows the live summon accelerator on Open Traycer SSH Dev as display-only, and none when disabled", () => {
    const controller = new DesktopTrayController(makeWindow(), trayImage(), {
      onEpicSelected: null,
      onCommand: null,
    });
